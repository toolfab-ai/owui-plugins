...
- The `@memory` prefix is reserved for the internal panel-command channel: any last user message starting with `@memory` (e.g. `@memorylane is fun`) is treated as a malformed panel command and answered with a polite explanation instead of being sent to the model as-is.
